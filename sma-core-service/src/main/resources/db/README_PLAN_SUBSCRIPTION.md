...
6. **Usage_events**
   ...
   - Có `status`:
     - `SUCCESS`: đang tính vào quota đã dùng
     - `FAIL`: request đã lỗi, không được tính vào quota
   - Business source không lưu trực tiếp trên event; dùng bảng con `usage_event_contexts`
   - Dùng để tính quota theo `Usage_limits`, nhưng chỉ count các event `SUCCESS`

7. **Usage_event_contexts**
   - Một `usage_event` có thể gắn nhiều context nguồn
   - Ví dụ `MATCHING_SCORE` có thể gắn đồng thời `JOB` và `RESUME`
...
3. **Tính usage**
   - Dựa trên `usage_events`, `usage_event_contexts`, và `usage_limits`
   - `PER_MONTH` reset theo chu kỳ subscription
   - Chỉ tính các event có `status = SUCCESS`
